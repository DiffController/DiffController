...
class OFMessageRepository(AbstractOFMessageRepository):
    """repository for all OpenFlow Msg"""
    def __init__(self,logger_name):
        super(OFMessageRepository, self).__init__()
        self.logger = getLogger(logger_name+'.of_msg_repository')

    # assign msg list for each local port(represent switch)
    def add(self, msg):
        self.repository.setdefault(msg.local_port, [])
        self.repository[msg.local_port].append(msg)
        self.logger.debug("added msg {} to repo {}".format(msg, self.repository))
    ...
    def pop(self, port, until=None, count=None):
        try:
            ...
            self.logger.error("Failed to pop from of repository (repo={})".format(self.repository))
            return None
...
class PacketInOutRepository(OFMessageRepository):
    ...
    def __init__(self,logger_name):
        super(PacketInOutRepository, self).__init__(logger_name)

    # here use dpid instead of local port
    def add(self, msg):
        if msg.datapath_id:
...
packet_in_out_repo = PacketInOutRepository("ofcapture")
